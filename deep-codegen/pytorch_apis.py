...
class spmmv_impl(th.autograd.Function):
    @staticmethod
    def forward(ctx, input, rowPtr, colInd, values):
        ...
        output = gp_apis.spmmv(input, output, rowPtr, colInd, values, degrees, V, F_in)
...
# Apply SpMMv
def spmmv(input, rowPtr, colInd, values):
    return spmmv_impl.apply(input, rowPtr, colInd, values)
